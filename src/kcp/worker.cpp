...
typhon::kcp::Worker::Worker(Server* s, int idx) noexcept
    : server_(s)
    , idx_(idx) {
...
typhon::kcp::Worker::~Worker() noexcept {
    for (int i = 0; i < MAX_RECV; ++i) {
...
void
typhon::kcp::Worker::run() noexcept {
...
int
typhon::kcp::Worker::output(const char *buf, int len, IKCPCB* kcpcb) noexcept {
...
void
typhon::kcp::Worker::init() noexcept {
...
void
typhon::kcp::Worker::release() noexcept {
...
void
typhon::kcp::Worker::remove_serv(tcp::Connector::Ptr c) noexcept {
...
void
typhon::kcp::Worker::on_event_handle(const ::epoll_event& ev) noexcept {
...
void
typhon::kcp::Worker::on_serv_handle(const ::epoll_event& ev) noexcept {
...
void
typhon::kcp::Worker::on_new_serv(core::QEvent* qe) noexcept {
...
void
typhon::kcp::Worker::update() noexcept {
    auto now = tnow_;
...
void
typhon::kcp::Worker::drain_qevent() noexcept {
