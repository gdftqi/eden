...
typhon::kcp::Server::Server(IEvent* ev) noexcept
    : event_(ev) {
...
typhon::kcp::Server::run() noexcept {
    ...
    // 3. 创建 KcpServer
    for (int i = 0; i < n; ++i) {
        auto s = std::make_unique<typhon::kcp::Worker>(this, i);
        ASSERT(s->fd() != typhon::core::INVALID_SOCKET, "创建 kcp worker 失败");
...
void
typhon::kcp::Server::update_serv() noexcept {
...
void
typhon::kcp::Server::on_serv_disconnected(const Event* ev) noexcept {
...
void
typhon::kcp::Server::on_user_connected(const Event*) noexcept {
    // TODO: redis 写入 用户路由表
}


void
typhon::kcp::Server::on_user_disconnected(const Event*) noexcept {
    // TODO: redis 删除 用户路由表
}