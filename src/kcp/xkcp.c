...
//   => kcp->mtu(KCP 数据报预算) = 链路MTU - IP头 - UDP头 - 8B信封;  mss = mtu - 24(XKCP_OVERHEAD)
 ...
 * @param len     数据长度
 * @return 成功 >=0(流模式返回已合并字节数); -1 参数错; -2 分片数超过接收窗口
 */
static int
...
// [XKCP] xkcp_setmtu 已取消: MTU 由 XKCP_MTU_DEF 编译期定死, mss/buffer/mac_buf 在 create 一次定型。
